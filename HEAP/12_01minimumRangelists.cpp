...
HERE WE USED MAX_HEAP FOR MAX VALUE ALSO THAT WE DON'T NEEDED
*/
class value{
    public:
    int data;
...
std::vector<int> smallestRange(std::vector<std::vector<int>> &nums){
    ...
    std::priority_queue<value> max_heap;
    ...
    for(int i = 0;i<R;i++){
        ...
        max_heap.push(value(nums[i][0],i,0));
        // store the size also
    ...
    while(!min_heap.empty()){
        ...
        max_heap.push(value(nums[x.row][x.col+1],x.row,x.col+1));
    }
